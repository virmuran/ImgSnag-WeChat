...
- 🖼 **大图预览（可缩放可拖动）+ 缩略图条**：左侧看大图，Ctrl+滚轮缩放（**以鼠标位置为中心**，想看哪放大哪）、双击在"适应窗口 / 100%"间切换，能放大到 1:1 确认真原图画质；放大超出窗口后**按住空格 + 拖动**平移；右侧勾选，支持按名称 / 文件大小 / 分辨率排序
...
│   └── test_ui_smoke.py       # 界面回归测试（118 项，离屏跑，含保存落盘端到端与缩放锚点/拖动）
